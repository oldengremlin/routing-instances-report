...
підробленого хоста без попередження. У відкритих мережах `StrictHostKeyChecking=no`
є суттєвою вразливістю.
...
`ReportGenerator` застосовує HTML-екранування (`&`, `<`, `>`, `"`, `'`)
до всіх даних що надходять від маршрутизаторів: імена інстансів, RD-рядки,
записи хостів, коди статусів. Це захист від XSS на випадок якщо конфігурація
пристрою містить символи HTML. Оскільки доступ до звіту є тільки у довіреного
персоналу команди — ризик є теоретичним, але додати захист коштує нічого.
